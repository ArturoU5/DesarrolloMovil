...
export default function DetailScreen({ route, navigation }: Props) {
  ...
  const cambiarEstado = async (nuevoEstado: Estado) => {
    try {
      await actualizarSolicitud(solicitud.id, { estado: nuevoEstado });
    } catch (error) {
  ...
  const guardarEdicion = async () => {
    ...
    try {
      await actualizarSolicitud(solicitud.id, { descripcion: descripcion.trim() });
  ...
  // El cliente "cancela" (queda como registro histórico con estado
  // CANCELADO); el administrador sí puede eliminar en forma permanente.
  const confirmarEliminar = async () => {
    try {
      if (esAdmin) {
        await eliminarSolicitud(solicitud.id);
      } else {
        await actualizarSolicitud(solicitud.id, { estado: ESTADOS.CANCELADO });
      }
      setDialogoVisible(false);
      navigation.goBack();
    } catch (error) {
      setDialogoVisible(false);
      Alert.alert('Error', 'No se pudo completar la acción. Intenta nuevamente.');
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.infoCard}>
          <InfoRow label="Cantidad de prendas" value={String(solicitud.cantidadPrendas)} />
          <InfoRow label="Precio estimado" value={`S/ ${solicitud.precio.toFixed(2)}`} />
          <InfoRow label="Teléfono" value={solicitud.telefono} />
          <InfoRow label="Dirección" value={solicitud.direccion} />
          <InfoRow
            label="Fecha de registro"
            value={formatearFechaHora(solicitud.fechaRegistro)}
          />
          <InfoRow
        ...
        {!esAdmin && !puedeGestionar && (
          <Text style={styles.avisoCliente}>
            Esta solicitud ya está siendo atendida, fue finalizada o ya se
            canceló, por lo que ya no se puede editar ni cancelar desde tu
            cuenta. Si necesitas hacer un cambio, comunícate directamente
            con RefreshProClean.
          </Text>
        )}
