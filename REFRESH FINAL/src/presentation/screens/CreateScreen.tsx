...
export default function CreateScreen({ navigation, route }: Props) {
  ...
  const servicioPreseleccionado = route.params?.servicioId ?? null;
  const servicioInicial = servicios.find((sv) => sv.id === servicioPreseleccionado);
  ...
  const estadoInicialForm: FormularioCreate = {
    ...
    cantidadPrendas: '',
    precio: servicioInicial ? String(servicioInicial.precio) : '',
  ...
  const [precioTocadoManualmente, setPrecioTocadoManualmente] = useState(false);
  ...
  // Al elegir un servicio, se autosugiere el precio de ese servicio (el
  // usuario lo puede sobrescribir; una vez que lo edita a mano, dejamos
  // de pisarlo automáticamente).
  const seleccionarServicio = (servicioId: string) => {
    actualizarCampo('servicioId', servicioId);
    if (!precioTocadoManualmente) {
      const servicio = servicios.find((sv) => sv.id === servicioId);
      if (servicio) {
        actualizarCampo('precio', String(servicio.precio));
      }
    }
  };
  ...
  const handleGuardar = async () => {
    ...
    const nuevaSolicitud = crearSolicitud({
      ...
      usuarioId: null,
      cantidadPrendas: Number(form.cantidadPrendas),
    ...
    setGuardando(true);
    try {
      await agregarSolicitud(nuevaSolicitud);
      ...
      Alert.alert(
        'No se pudo guardar',
        'Ocurrió un problema al guardar la solicitud en el dispositivo. Intenta nuevamente.'
      );
    } finally {
      setGuardando(false);
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.serviciosWrap}>
          {servicios.map((sv) => (
            <TouchableOpacity
              ...
              onPress={() => seleccionarServicio(sv.id)}
            >
        ...
        <View style={styles.filaDoble}>
          <View style={{ flex: 1, marginRight: 8 }}>
            <InputField
              ...
              value={form.cantidadPrendas}
              onChangeText={(v) => actualizarCampo('cantidadPrendas', v)}
              error={errores.cantidadPrendas}
              placeholder="Ej. 5"
          ...
          <View style={{ flex: 1, marginLeft: 8 }}>
            <InputField
              label="Precio estimado (S/)"
              value={form.precio}
              onChangeText={(v) => {
                setPrecioTocadoManualmente(true);
                actualizarCampo('precio', v);
              }}
        ...
        <Text style={[styles.label, { marginTop: 4 }]}>Prioridad</Text>
        ...
        <InputField
          ...
          placeholder="Detalla las prendas, indicaciones especiales..."
          multiline
        />
...
const styles = StyleSheet.create({
  ...
  filaDoble: { flexDirection: 'row', marginTop: 8 },
