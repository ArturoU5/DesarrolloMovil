 ...
 * falla la petición".
 *
 * RefreshProClean no tiene todavía un backend propio de catálogo, así
 * que para esta entrega preliminar se usa Fake Store API (pública, sin
 * API key) como catálogo de referencia de productos/insumos sugeridos
 * (ej. detergentes, accesorios). Cuando el negocio tenga su propio
 * catálogo, solo habría que cambiar API_URL por el endpoint real: el
 * resto de la app (pantalla, loading, error) no cambiaría.
 */
